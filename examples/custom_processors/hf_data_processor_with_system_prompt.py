...
PROCESSOR_NAME = "hf_data_processor_with_system_prompt"


def GET_SYSTEM_PROMPT(task: str) -> dict:
    answer_wrapper = r"\boxed{}"

    return {
        "role": "system",
        "content": r"""Solve the problem step by step.
Put your reasoning between <think> and </think>.
Afterwards, explain your logic then put your the final answer in """
        + answer_wrapper
        + """ after the reasoning.""",
    }


def hf_data_processor_with_system_prompt(
    ...
    message: list[str] = tokenizer.apply_chat_template(  # type: ignore
        [GET_SYSTEM_PROMPT(task=datum_dict["task_name"]), user_message],
        tokenize=False,
...
if PROCESSOR_NAME not in PROCESSOR_REGISTRY:
    register_processor(PROCESSOR_NAME, hf_data_processor_with_system_prompt)
